...
#### 주요 개발 항목
- [x] **관리자 웹 서비스 개발** (신규) - **🔄 진행 중 (Week 1-3 진행 중)**
  - [x] **프로젝트 셋업 (2025.10.19)** - **✅ 완료**
    - React 19 + Vite 7.1.10 + TypeScript 환경 구성
    - Tailwind CSS 4.0 (shadcn/ui 불필요)
  ...
  - [x] **명함 관리 (Week 3)** - **🔄 진행 중 (2025.11.13)**
    - [x] 명함 목록 페이지 (CardsPage)
      ...
      - 다중 선택 및 일괄 작업 바
      - 페이지네이션 (50개/페이지)
    - [x] 명함 상세 페이지 (CardDetailPage)
      - 사용자 정보 및 명함 정보 카드
      - 통계 차트 (방문자 추이, 디바이스/브라우저)
      - 최근 방문자 목록 (50개)
      - 명함 활성/비활성 토글
      - 테마 변경 드롭다운
...
- v2.3 (2025.11.13): Phase 3 관리자 웹 서비스 Week 3 진행 중 - 명함 관리 기능
  - ✅ **관리자 명함 관리 개발 진행 중 (Week 3)**
    - 명함 목록 페이지 (CardsPage) 구현 완료
      - 검색/필터/정렬 기능 (테마, 상태, 생성일/조회수)
      - 그리드뷰/테이블뷰 전환 UI
      - 다중 선택 및 일괄 작업 (활성/비활성/테마 변경)
      - 페이지네이션 (50개/페이지)
    - 명함 상세 페이지 (CardDetailPage) 구현 완료
      - 사용자 정보 및 명함 상세 정보 표시
    ...
    - **명함 편집 모달 (CardEditModal) 구현 완료** ✅
      ...
      - 프로필 이미지 & 회사 로고 업로드 (Supabase Storage)
      - React Hook Form + Zod 검증
      - 순수 HTML + Tailwind CSS (shadcn/ui 없이)
      - 20+ 필드 지원, 5개 테마 선택
    - API 함수 구현
      - fetchCards (필터/페이지네이션)
      - fetchCard (상세 정보)
      - fetchCardDetailStats (통계)
      - updateCardStatus, updateCardTheme
      - deleteCard, bulkUpdateCards
      - **updateCard (전체 필드 수정)** ✅
    - 타입 시스템 정리
      - CardWithUser, CardWithStats 타입 정의
      - admin.ts 타입 파일 체계화
